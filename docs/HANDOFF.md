...
## 0. READ FIRST — current issue + immediate deliverable

**The Streamlit app is too slow / times out.** Root cause (profiled): scoring the
2,234-peptide library takes ~71 s, and **~80% is `hemolysis_prior`** (24.9 ms ×
2,234 ≈ 56 s). Two fixable reasons: (1) it calls `compute_descriptors` for **all
210 features** per peptide but uses only ~25; (2) it runs the ML model **one
peptide at a time** (2,234 separate `predict_proba` calls). Also structural: the
`usable_delivery` ranking is **anchor-independent**, so the full scan re-runs on
every app interaction even though the intrinsic scores never change.

**Fix plan (performance only — must NOT change any score):**
1. In `scoring/disruption.py`: featurize only the model's ~25 features (not all
   210), and add a **batch** `hemolysis_prior_batch(seqs)` that calls
   `predict_proba` once on a stacked array.
2. Compute descriptors **once** per candidate and share the dict across
   surface/insertion/hemolysis instead of each recomputing.
3. **Cache the scored library** in the app (the intrinsic scores don't depend on
   the anchor) so interactions are instant.
Target: ~71 s → < 5 s. Verify with `python -m cpp_ai.benchmark` (scores unchanged)
